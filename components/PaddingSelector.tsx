    ...
    <div>
      ...
      <div className="flex gap-6">
        {paddings.map((padding, i) => (
          <button key={i} onClick={() => changePadding(padding)} 
          className={`h-[37px] flex items-center justify-center text-sm px-2 ${currentPadding === padding && 'bg-[#3C3C3C] rounded-md  text-white'} hover:text-white ease-linear transition-all duration-300`}>
            {padding}
          </button>
        ))}
      </div>
